...
'''
 ...
 '''
    ...
    def plot(self, vals=np.linspace(-1,1,100), **kwargs):
        ...
        num = []
        for i in vals:
            num.append(self.evaluate(i))
        return plt.plot(vals, num, **kwargs)
        #aise NotImplementedError("plot")

    def taylor_series(self, x0, k):
        """
        Returns the Taylor series of f centered at x0 truncated to degree k
        """
        f0 = AbstractFunction.evaluate(self, x0)#this is the first term, special cased to avoid 0 power
        terms = []*(k+1)#k degree Taylor series has k+1 terms
        terms[0] = f0
        offset = Polynomial(1, -x0)#defines the (x-x0) term in taylor
        for i in range(k+1):#loop puts each element of the sum in terms[]
            if i == 0:#base case
                pass
            else:
                ith_deriv = self#initialize i_th deriv
                for j in range(i):#takes i_th derivative of f
                   ith_deriv = AbstractFunction.derivative(ith_deriv)
                terms[i] = (1/np.math.factorial(i)) * ith_deriv * (offset ** i)#completed term
        t_series = np.sum(terms)#sum all of the terms in taylor
        return t_series
...
if __name__ == '__main__':
    ...
    sinExp = Sin(Exponential(Polynomial(1,0)));
